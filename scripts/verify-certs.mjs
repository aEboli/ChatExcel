...
import { createSecureContext, getCACertificates } from "node:tls";
...
const trustedBySystem = getCACertificates("system").some((pem) => {
  try {
    return new X509Certificate(pem).fingerprint256 === caCertificate.fingerprint256;
  } catch {
    return false;
  }
});
