...
- 在输入框底部提供模型、思考等级、上下文占用和审批模式控制，并支持添加、预览和删除图片附件。
...
- 输入框除文件选择器外支持直接粘贴剪贴板图片，并复用同一附件校验、压缩、预览和删除流程。
...
- `taskpane-experience`: 提供 ChatExcel 的工作簿感知界面、紧凑审计记录、模型控制、设置、附件和历史修改保护。
...
- 默认读取 `~/.codex/config.toml`；关闭系统配置后，自定义 API Key 只以 Windows 当前用户 DPAPI 密文持久化，明文仅在本地服务进程内使用。用户指令、必要的工作簿上下文和主动添加的图片只发送到当前选择的模型提供方。
