...
### Requirement: 支持多协议图片输入
系统 SHALL 接受最多四张 PNG、JPEG 或 WebP 图片作为用户消息附件，MUST 验证数据 URL、单图大小和总大小，并 SHALL 由协议适配器转换为对应的图片内容且不写入磁盘。

#### Scenario: 带标注图片发送任务
- **WHEN** 用户添加有效图片并发送带文字的工作簿任务
- **THEN** 模型请求包含对应 `input_text` 和 `input_image` 内容，任务窗格可在发送前删除任一图片
